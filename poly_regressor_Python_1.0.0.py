...
num_alphas = args.num_alphas  # Use the command-line argument
# num_alphas = 20  # Comment out the hardcoded value
...
with mlflow.start_run(run_name=run_name):
    ...
    for alpha in range(0, alpha_max, 2):
        ...
        with mlflow.start_run(run_name=run_num, nested=True):
            ...
            mlflow.log_param("degree", order)  # Log polynomial degree
            mlflow.log_metric("mse", score)
            mlflow.log_metric("avg_delay", np.sqrt(score))  # Log average delay
        if score < score_min:
            score_min = score
...
with mlflow.start_run(run_name="Final Model - Test Data"):
    # Log parameters
    mlflow.log_param("alpha", parameters[0]/10)  # Optimal alpha from the loop
    mlflow.log_param("degree", order)  # Polynomial degree
    # Log metrics
    mlflow.log_metric("Test MSE", score)  # Mean squared error from test data
    mlflow.log_metric("Test Avg Delay", np.sqrt(score))  # Average delay in minutes
    # Log artifacts
    mlflow.log_artifact("polynomial_regression.txt")  # Model training log
    mlflow.log_artifact("model_performance_test.jpg")  # Performance plot
    mlflow.log_artifact("finalized_model.pkl")  # Trained model
    mlflow.log_artifact("process_data_log.txt")  # Combined import and clean log
mlflow.end_run()
logging.shutdown()
